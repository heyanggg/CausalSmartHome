...
STAGE3B_ROOT = OUT_ROOT / "fr_st_stage3b_ad"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Summarize FR-ST Stage 3B repeat robustness runs")
    parser.add_argument(
        "--run",
        action="append",
        default=[],
        metavar="NAME=TAG",
        help="Repeat mapping, e.g. seed2024=fr_st_codex_calibrated_v3. Can be repeated.",
    )
    parser.add_argument("--out-csv", type=Path, default=OUT_ROOT / "fr_st_stage3b_repeat_summary.csv")
    parser.add_argument("--out-json", type=Path, default=OUT_ROOT / "fr_st_stage3b_repeat_summary.json")
    parser.add_argument("--out-md", type=Path, default=OUT_ROOT / "fr_st_stage3b_repeat_summary.md")
...
def main() -> None:
    args = parse_args()
    runs = args.run or [
        "seed2024=fr_st_codex_calibrated_v3",
        "seed2025=fr_st_codex_calibrated_seed2025",
        "seed2026=fr_st_codex_calibrated_seed2026",
    ]
    per_repeat: list[dict[str, Any]] = []
    for item in runs:
        name, tag = item.split("=", 1)
        metrics_json = STAGE3B_ROOT / tag / "metrics.json"
