...
DEFAULT_SOURCE_TRAIN = Path("/home/heyang/projects/SmartGen/SmartGen/IoT_data/fr/winter/trn.pkl")
DEFAULT_EVAL_SOURCE_TRAIN = Path("/home/heyang/projects/SmartGen/behavior_prediciton_baseline/SASRec/baseline_data/fr/winter/trn.pkl")
DEFAULT_TARGET_REAL = Path("/home/heyang/projects/SmartGen/parameter_study/test/fr/spring/split_test.pkl")
DEFAULT_ORIGINAL_RAW = SMARTGEN_ROOT / "IoT_data/fr/spring/fr_spring_generation_SPPC_th=0.918_gpt-4o_seq.pkl"
DEFAULT_ORIGINAL_TOF = SMARTGEN_ROOT / "filter_data/fr/spring/fr_spring_generation_SPPC_th=0.918_gpt-4o_seq_filter_true.pkl"
...
class GroupPaths:
    ...
    def raw_pkl(self) -> Path:
        return self.work_dir / f"IoT_data/fr/spring/fr_spring_generation_SPPC_th=0.918_{self.model_tag}_seq.pkl"

    @property
    def tof_pkl(self) -> Path:
        return self.work_dir / f"filter_data/fr/spring/fr_spring_generation_SPPC_th=0.918_{self.model_tag}_seq_filter_true.pkl"
...
def sentence_for_fr_st() -> str:
    return "The previous environment is winter. The changed environment is warm spring."


def discover_categories(smartgen_root: Path, threshold: str, method: str) -> list[str]:
    base = smartgen_root / "IoT_data/fr/winter"
    categories: list[str] = []
...
def prepare_prompt_checks(args: argparse.Namespace) -> dict[str, Any]:
    ...
    out_dir = run_root / "fr_st_prompt_check"
    out_dir.mkdir(parents=True, exist_ok=True)
    categories = discover_categories(args.smartgen_root, args.threshold, args.method)
    if not categories:
        raise FileNotFoundError("no FR-ST SmartGen prompt categories found")
    ...
    device_mapping = load_id_name_mapping(args.smartgen_root / "dictionary.py", preferred_names=("fr_devices_dict", "device_dict"))
    ...
    payload10["scenario"] = "fr_st"
    hints10 = format_causal_hints_for_prompt(payload10)

    action_transition = json.loads((args.smartgen_root / "IoT_data/fr/winter/action_transitions.json").read_text(encoding="utf-8"))
    device_control_dict = (args.smartgen_root / "fr_keys_best.txt").read_text(encoding="utf-8")
    sample_category = categories[0]
    user_sequence = load_pickle(args.smartgen_root / f"IoT_data/fr/winter/trn_day_{sample_category}_{args.method}_th={args.threshold}_text.pkl")
...
def prepare_group_workdir(paths: GroupPaths) -> None:
    for relative in ("IoT_data/fr/spring", "filter_data/fr/spring", "check_model"):
...
def codex_authored_response(
    ...
    """Codex-authored FR-ST generations in SmartGen's response format.
...
def codex_calibrated_response(
    ...
    """Generate FR-ST text while matching SmartGen's spring style envelope.
    ...
    FR-ST TOF baseline only as a style bank for event length, device/action
    ...
    device_by_id = {value: key for key, value in dictionary.fr_devices_dict.items()}
    action_by_id = {value: key for key, value in dictionary.fr_actions.items()}
    action_by_device: dict[str, list[str]] = {}
    for action in dictionary.fr_actions:
    ...
    def maybe_rewrite(events: list[list[str]], sample_idx: int) -> list[list[str]]:
        ...
        if not rewritten:
            return rewritten
        # Increase FR-ST spring signals without erasing the source style.
...
def generate_group(paths: GroupPaths, prompt_kind: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    for category in categories:
        prompt_input = args.smartgen_root / f"IoT_data/fr/winter/trn_day_{category}_{args.method}_th={args.threshold}_text.pkl"
        ...
        save_pickle(
            paths.work_dir / f"IoT_data/fr/spring/fr_spring_generation_day_{category}_{args.method}_th={args.threshold}_{paths.model_tag}.pkl",
            response,
        )
...
def run_tof(paths: GroupPaths, args: argparse.Namespace) -> dict[str, Any]:
    ...
    dictionaries = [dictionary.dayofweek_dict, dictionary.hour_dict, dictionary.fr_devices_dict, dictionary.fr_actions]
    categories = discover_categories(args.smartgen_root, args.threshold, args.method)
    ...
    with open(log_path, "w", encoding="utf-8") as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        ...
        with pushd(paths.work_dir):
            extract("fr", "spring", args.threshold, args.method, paths.model_tag, categories)
            transnum("fr", "spring", args.threshold, args.method, paths.model_tag, categories, dictionaries)
            security_check("fr", "spring", args.threshold, args.method, paths.model_tag)
...
def write_summary(args: argparse.Namespace, group_paths: Sequence[GroupPaths], prompt_check: dict[str, Any] | None) -> None:
    ...
    run_root = output_root(args)
    csv_path = run_root / "fr_st_stage3a_summary.csv"
    md_path = run_root / "fr_st_stage3a_summary.md"
    fieldnames = [
        "group",
        "status",
        "causal_coverage",
    ...
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    write_json(run_root / "fr_st_stage3a_summary.json", {"rows": rows, "prompt_check": prompt_check})
...
def command_for_group(stage: str, group: str, args: argparse.Namespace) -> str:
    ...
    return (
        f"{sys.executable} scripts/run_stage3a_gcad_gss_fr_st.py --stage {stage} --groups {group} "
        f"--threshold {args.threshold} --method {args.method} --llm-model {args.llm_model}{offline}{require_cuda}"
...
def load_or_prepare_prompt_check(args: argparse.Namespace) -> dict[str, Any]:
    prompt_check_path = output_root(args) / "fr_st_prompt_check/prompt_check.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Stage 3A FR-ST GCAD-GSS prompt quality evaluation")
    ...
    parser.add_argument("--threshold", default="0.918")
...
def main() -> None:
    ...
    group_paths = {
        "original": GroupPaths("original", run_root / "fr_st_original", run_root / "fr_st_original/smartgen_work", f"gpt-4o_stage3a_original{suffix}"),
        "enhanced": GroupPaths("enhanced", run_root / "fr_st_enhanced", run_root / "fr_st_enhanced/smartgen_work", f"gpt-4o_stage3a_enhanced{suffix}"),
    }
    prompt_check = None
    if args.stage == "check":
        prompt_check = prepare_prompt_checks(args)
        print(f"prompt_check: {run_root / 'fr_st_prompt_check/prompt_check.json'}")
    elif args.stage in {"generate", "all"}:
        prompt_check = load_or_prepare_prompt_check(args)
        print(f"prompt_check: {run_root / 'fr_st_prompt_check/prompt_check.json'}")
    elif (run_root / "fr_st_prompt_check/prompt_check.json").exists():
        prompt_check = json.loads((run_root / "fr_st_prompt_check/prompt_check.json").read_text(encoding="utf-8"))
    ...
    if args.stage in {"tof", "all"}:
        for group in selected:
            paths = group_paths[group]
            if (paths.out_dir / "smartgen_tof.pkl").exists() and args.offline_generator != "codex":
    ...
    if args.stage in {"summary", "all"}:
        ...
        print(f"summary: {run_root / 'fr_st_stage3a_summary.md'}")
        print(f"summary_csv: {run_root / 'fr_st_stage3a_summary.csv'}")
