...
CSH_ROOT = REPO_ROOT
DEFAULT_STAGE3A_TAG = "fr_st_thr_0p001"
DATASET = "fr"
ENV = "spring"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Run Stage 3B FR-ST AD check for GCAD-GSS prompt outputs")
    ...
    parser.add_argument("--out-dir", type=Path, default=CSH_ROOT / "outputs/gcad_gss/fr_st_stage3b_ad")
    parser.add_argument(
        "--attack-pkl",
        type=Path,
        default=SMARTGEN_ROOT / "anomaly_detection_pipeline/attack/fr/labeled_fr_spring_attack_heater.pkl",
    )
    parser.add_argument(
        "--target-test-pkl",
        type=Path,
        default=SMARTGEN_ROOT / "anomaly_detection_pipeline/test/fr/spring/test.pkl",
    )
...
def main() -> None:
    ...
    original_tof = args.original_tof_pkl or (stage3a_root / "fr_st_original/smartgen_tof.pkl")
    enhanced_tof = args.enhanced_tof_pkl or (stage3a_root / "fr_st_enhanced/smartgen_tof.pkl")
