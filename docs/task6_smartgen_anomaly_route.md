...
4. 在 CausalSmartHome 中执行 CUDA-first 的 train / find threshold / evaluate 循环，默认使用第 0 张 NVIDIA GPU。
...
CausalSmartHome wrapper 已绕开 SmartGen 原脚本里的硬编码 `.cuda()`，并默认用 CUDA 调用第 0 张 GPU 跑 FR winter -> spring 的 SmartGen Transformer Autoencoder 评估。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| unfiltered_filter_true | 125 | 100 | 25 | 1.3921 | 0.9886 | 0.7311 | 0.8406 |

这个数值与早期 SmartGen 原生结果对齐，说明 GPU 复现实验口径是可信的。

输出目录：

```text
outputs/fr_winter_to_spring_device_h5e-05/smartgen_anomaly_eval_gpu/
```

### 公共 validation 校准

用未过滤 `filter_true` 的 validation split 作为公共阈值校准集后，三档 causal filter 结果为：
...
```text
Causal filtering can improve SmartGen synthetic data under the native Transformer Autoencoder evaluation when paired with common validation calibration.
```

输出目录：

```text
outputs/fr_winter_to_spring_device_h5e-05/smartgen_anomaly_sweep_eval_gpu_common_vld/
```
